...
def _try_tune(cart, lo_ghz, msg, skip_servo_pa, log):
...
def _try_att(cart, photonics, lo_ghz, att, skip_servo_pa, delay_secs, sleep, log):
    ...
    if cart.state['pll_unlock']:
        _try_tune(cart, lo_ghz, 'att %d'%(att), skip_servo_pa, log)
...
def tune(cart, agilent, photonics, lo_ghz, pll_range=[-.8,-2.5],
    ...
    _try_tune(cart, lo_ghz, 'att %d, dbm %.2f'%(att,dbm), skip_servo_pa, log)
    ...
    while photonics and (cart.state['pll_unlock'] or cart.state['pll_if_power'] > pll_range[0]) and att > att_min:
        ...
        _try_att(cart, photonics, lo_ghz, att, skip_servo_pa, delay_secs, sleep, log)
